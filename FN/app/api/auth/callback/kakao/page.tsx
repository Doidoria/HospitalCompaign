    ...
    if (code && !isCalledRef.current) {
      ...
      authApi.loginWithKakao(code)
        ...
        .catch(async (err) => {
          ...
          // 🚨 백엔드에서 커스텀 예외(REQUIRE_ACCOUNT_LINK)가 떨어졌을 때 가로채기
          const errorResponse = err.response?.data;
          
          if (errorResponse?.code === 'REQUIRE_ACCOUNT_LINK') {
            ...
            YesAlert.fire({
              ...
              html: `입력하신 휴대폰 번호로 이미 가입된 일반 계정이 존재합니다.<br/>
                     카카오 로그인 정보(<b>${existingEmail}</b>)를 우선하여 
              ...
              confirmButtonColor: '#FEE500', // 카카오 브랜드 옐로우
              cancelButtonColor: '#94A3B8'
            }).then(async (result) => {
              if (result.isConfirmed) {
                try {
                  // 사용자가 동의 시 최종 연동 승인 API 가동
            ...
            YesAlert.fire({ icon: 'error', title: '로그인 실패', text: '카카오 로그인 중 오류가 발생했습니다.' });
            router.push('/login');
          }
        });
    }
